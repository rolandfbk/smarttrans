...
{
	header("location: manage-user.php?act=duplicate");
}
else
{
	$sql=" INSERT INTO mobile_prefix (PREFIX) values ('DRV')";
	$result=ExecuteNonQuery ($sql);


	$sql1="select * from mobile_prefix order by ID desc limit 1";
	...
	$sql2=" INSERT INTO controller_user (REFERENCE_NUMBER,FIRST_NAME,SURNAME,EMAIL,PASSWORD,PHONE,USER_TYPE,CREATED_AT,UPDATED_AT) values ('$orderNumber','$fn','$surname','$email','$password','$phone','mobile',now(),now())";
	...
	if($result2)
	{
		...
		header("location: con_manage-user.php?act=mobilevalid");
	}
	else
	{
		header("location: con_manage-user.php?act=mobileinvalid");
	}
}
