...
$sql3="select * from tanker where VEHICLE_REG='$vehicle_reg'";
$result3 = ExecuteQuery($sql3);

if(mysqli_num_rows($result3)==1)
{
	header("location: con_tanker.php?act=duplicate");
}
else
{
	$sql2=" INSERT INTO tanker (VEHICLE_REG,VEHICLE_MAKE,VIN_NO,EXPIRY_DATE,CREATED_AT,UPDATED_AT) values ('$vehicle_reg','$vehicle_make','$vin_no','$expiry_date',now(),now())";
	...
	if($result2)
	{
		...
		header("location: con_tanker.php?act=trailervalid");
	}
	else
	{
		header("location: con_tanker.php?act=trailerinvalid");
	}
}
