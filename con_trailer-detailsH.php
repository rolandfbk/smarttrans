...
$sql3="select * from trailer where VEHICLE_REG='$vehicle_reg'";
$result3 = ExecuteQuery($sql3);

if(mysqli_num_rows($result3)==1)
{
	$sql="update trailer set VEHICLE_MAKE='$vehicle_make',VIN_NO='$vin_no',EXPIRY_DATE='$expiry_date',UPDATED_AT=now() where trailer.ID=$id";

	$result=ExecuteNonQuery ($sql);

	if($result)
	{
		header("location: con_trailer-details.php?id=$id&act=valid");
	}
	else
	{
		header("location: con_trailer-details.php?id=$id&act=invalid");
	}
...
{
	$sql="update trailer set VEHICLE_REG='$vehicle_reg',VEHICLE_MAKE='$vehicle_make',VIN_NO='$vin_no',EXPIRY_DATE='$expiry_date',UPDATED_AT=now() where trailer.ID=$id";
