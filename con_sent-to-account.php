...
if($result)
{
	$sql4=" INSERT INTO activity_log (WORK_ORDER_NUMBER,COMMENT,TIME,SYSTEM_TIME,USER,CREATED_AT) values ('$orderNumber','Work Order sent to Account by $user',now(),now(),'$user',now())";
	...
	header("location: con_work-order-manage.php?id=$id&act=accountvalid");
}
else
{
	header("location: con_work-order-manage.php?id=$id&act=accountinvalid");
}
