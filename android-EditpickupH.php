...
{
	$sql2=" INSERT INTO pickup (WORK_ORDER_NUMBER,PRODUCT,QUANTITY,DESCRIPTION,TONNAGE,TIME,PICKED_UP_BY,CREATED_AT) values ('$id','$product','$quantity','Amended','$tonnage','$pickup_time','$user',now())";

	$result2=ExecuteNonQuery ($sql2);

	if($result2)
	{
		$sql3=" INSERT INTO activity_log (WORK_ORDER_NUMBER,COMMENT,TIME,SYSTEM_TIME,USER,CREATED_AT) values ('$id','pickup confirmation with amendment','$pickup_time',now(),'$user',now())";
