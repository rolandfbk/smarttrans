...
{
	header("location: con_trailer.php?act=duplicate");
}
else
{
	$sql2=" INSERT INTO trailer (VEHICLE_REG,VEHICLE_MAKE,VIN_NO,EXPIRY_DATE,CREATED_AT,UPDATED_AT) values ('$vehicle_reg','$vehicle_make','$vin_no','$expiry_date',now(),now())";
	...
	if($result2)
	{
		...
		header("location: con_trailer.php?act=trailervalid");
	}
	else
	{
		header("location: con_trailer.php?act=trailerinvalid");
	}
}
