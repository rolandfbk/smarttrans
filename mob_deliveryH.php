...
$signed_by1 = ucfirst(strip_tags(stripslashes($_POST['signed_by'])));
$output1 = (strip_tags(stripslashes($_POST['output'])));
...
$output = filter_var($output1, FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_HIGH);
...
$sql2=" INSERT INTO delivery (WORK_ORDER_NUMBER,DESCRIPTION,DATE,TIME,SIGNED_BY,SIGNATURE,DELIVERED_BY,CREATED_AT) values ('$orderNumber','$delivery_description','$delivery_date','$delivery_time','$signed_by','$output','$user',now())";
...
if($result2)
{
	...
	header("location: mob_work-order-details.php?id=$id&act=deliveryvalid");
}
else
{
	header("location: mob_work-order-details.php?id=$id&act=deliveryinvalid");
}
