...
{
	header("location: controller.php?act=duplicate");
}
else
{
	$sql=" INSERT INTO controller_prefix (PREFIX) values ('CONT')";
	...
	$sql2=" INSERT INTO controller_user (REFERENCE_NUMBER,FIRST_NAME,SURNAME,EMAIL,PASSWORD,PHONE,USER_TYPE,CREATED_AT,UPDATED_AT,PRINT_FROM) values ('$orderNumber','$fn','$surname','$email','$password','$phone','controller',now(),now(),now())";
	...
	if($result2)
	{
		...
		header("location: controller.php?act=valid");
	}
	else
	{
		header("location: controller.php?act=invalid");
	}
}
